...
var top = window, cover, doc, dragDiv, dragIframe, ZIndex, dcount = 0;

while (top.parent != top && top.parent.document.getElementsByTagName('frameset').length == 0)
	top = top.parent;
...
J.ui.dialog = function( opts, elem )
{
	...
	if( !dragDiv )
	{
		dragIframe =  J('<iframe hideFocus="true" frameborder="0" src="about:blank" style="position:absolute;top:0px;left:0px;' +
		    'background-color:#999;display:none;"></iframe>',doc)
			.appendTo('body').bind('contextmenu', function(ev){ ev.preventDefault(); })[0];
	...
	this.SetIFramePage = function()
	{
		...
		text = [
			'<div id="', r.id, '" class="lhgdig" style="width:', r.width, 'px;height:', r.height, 'px;">',
				...
				'</table>', iframe,
			'</div>',
			'<iframe id="', r.id, '_ifr" frameborder=0 style="position:absolute;background-color:blue;width:', r.width, 'px;height:', r.height, 'px;"></iframe>'
		].join('');
	...
	this.ShowDialog = function()
	{
		...
		this.dlg = J(html,doc).css({
		    position: fixpos, zIndex: zIndex+1
		}).appendTo(doc.body)[0];
		...
		//
		var ifr = this.dlg.nextSibling;
		ifr.style.left = this.dlg.offsetLeft;
		ifr.style.top = this.dlg.offsetTop;
		this.dlgifr = J(ifr).css({
			zIndex: zIndex
		}).appendTo(doc.body)[0];
	...
	this.reContentSize = function( dig )
	{
		...
		J('#'+r.id+'_ifr').css({
		    width: dig.style.width, height: dig.style.height
		});
	...
	this.initDrag = function( elem )
	{
		...
		function moveHandler(ev)
		{
			...
			J(dragIframe).css({ left: curpos.x + 'px', top: curpos.y + 'px' });
		};
		...
		function upHandler(ev)
		{
			...
			J(self.dlgifr).css({ left: self.dlg.offsetLeft + 'px', top: self.dlg.offsetTop + 'px' });
		};
		...
		J(elem).bind( 'mousedown', function(ev){
			...
			J(dragDiv).css({
				...
				top: curpos.y + 'px', zIndex: parseInt(ZIndex,10) + 3, display: ''
			});
			J(dragIframe).css({
	...
	this.ShowCover = function()
	{
	    if( !cover )
		{
			var html = [ '<div style="position:absolute;top:0px;left:0px;',
					...
					'background-color:#fff;"></div>' ].join('');
			
			cover = J(html,doc)[0]
			cover.style.opacity = 0.5;
			cover.style.filter='alpha(opacity=50)';
			J(cover).appendTo(doc.body)[0];
	...
	this.cancel = function()
	{
		...
		if( self.opt.fixed && J.browser.ie && !J.browser.i7 )
		{
			...
			J(self.dlgifr).remove(); self.dlgifr = null;
		}
		if( cover )
		{
			self.AFGray(0);
	...
	this.AFGray = function(b){
		var v = false;
		if (typeof r.AF == 'function') {
			v = r.AF(b);
		}
