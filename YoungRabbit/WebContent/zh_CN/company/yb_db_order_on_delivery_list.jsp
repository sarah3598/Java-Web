...
<body>
		...
		<div class="row">
			...
			<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
				...
				<ul class="nav nav-tabs">
					...
					<li class="active"><a href="<%=path%>/company/orders_manage?method=order_on_delivery_list"> 配送中订单 </a></li>
	...
	<script>
		<%String order_list_json = (String) request.getAttribute("order_on_delivery_list");
			if (order_list_json == "") {
				order_list_json = "[]";
			}%>
		...
		function upd_order(ORDER_ID,GOOD_ID,ORDER_NO,HAPPEN_DATE,HAPPEN_TIME,TITLE,BUY_NUM,PRODUCT_WEIGHT,SHIPPING_PRICE,ADDRESS_ID,IS_INVOICE,INVOICE_CLIENT){
			var create_time=HAPPEN_DATE+" "+HAPPEN_TIME;
			var tweight=BUY_NUM*PRODUCT_WEIGHT;			
			var address="获取地址";
			var INVOICE_CLIENT=IS_INVOICE==1?INVOICE_CLIENT:"不开发票";
		...
		function pagination(current_page,page_size){
			window.location.href="<%=path%>/company/orders_manage?method=order_on_delivery_list&current_page="+current_page+"&page_size="+page_size;
		}
		...
		$( "#PAGE_SIZE" ).change(function() {
			...
			window.location.href="<%=path%>/company/orders_manage?method=order_on_delivery_list&page_size="+page_size;
		});
