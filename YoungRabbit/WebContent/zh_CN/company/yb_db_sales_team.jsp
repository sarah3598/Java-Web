...
<%@include file="yb_db_nav.jsp"%>
				...
				<table class="table table-striped" style="border: 1px solid #eee;">
					...
					<script id="tpl" type="text/x-handlebars-template">
						...
						<tr>
							...
							<td>{{SUM}}</td>
							<td>
							<!--<button type="button" class="btn btn-sm btn-success" onclick="team_detail('{{TEAM_ID}}','{{TYPE}}','{{USERNAME}}');">结构</button>-->
								<button type="button" class="btn btn-sm btn-primary" onclick="adjust('{{TEAM_ID}}','{{ROLE}}','{{USER_NAME}}');">人员调整</button>
							<!--<button type="button" class="btn btn-sm btn-danger" onclick="close_team('{{TEAM_ID}}');">关闭</button>-->
							<!--<button type="button" class="btn btn-sm btn-danger" onclick="del_train('{{TEAM_ID}}');">删除</button>-->
							</td>
						</tr>
						{{/each}}
					</script>		  
				...
				<table class="table table-striped" style="border: 1px solid #eee;">
					<thead>
						<tr>
							...
							<th>销量(RM)</th>
							<th>操作
								<!--<button type="button" class="btn btn-xs btn-primary" onclick="show(5);">添加经理团队</button>-->
							</th>
						</tr>
					</thead>
					<tbody id="test2">
					  <script id="tpl2" type="text/x-handlebars-template">
						{{#each this}}
							<tr>
							  ...
							  <td>{{GROUP_SALE}}</td>
							  <td>
								<button type="button" class="btn btn-sm btn-success" onclick="team_move('{{TEAM_ID}}','{{USER_ID}}','{{USERNAME}}');">团队迁出</button>
								<button type="button" class="btn btn-sm btn-success" onclick="add_sales('{{TEAM_ID}}','{{USER_ID}}','{{USERNAME}}');">添加销售</button>
								<button type="button" class="btn btn-sm btn-danger" onclick="team_close('{{TEAM_ID}}','{{USER_ID}}');">关闭</button>
								<button type="button" class="btn btn-sm btn-danger" onclick="team_del('{{TEAM_ID}}','{{USER_ID}}','{{ROLE}}');">删除</button>
							  </td>
							</tr>
							{{#each members}}
							<tr>
							 <td>{{V_NAME}}</td>
							  ...
							  <td>{{PERSONAL_SALE}}</td>
							  <td>
								<button type="button" class="btn btn-sm btn-success" onclick="member_move('{{TEAM_ID}}','{{USER_ID}}','{{USERNAME}}');">调岗</button>
								<button type="button" class="btn btn-sm btn-danger" onclick="dimission('{{TEAM_ID}}','{{USER_ID}}');">离职</button>
								<button type="button" class="btn btn-sm btn-danger" onclick="member_del('{{TEAM_ID}}','{{USER_ID}}');">删除</button>
							  </td>
							</tr>
							{{/each}}
						{{/each}}
	...
	<script>
		...
		function team_list(){
			...
			window.location.href="<%=path%>/company/team_manage?method=team_list&YM="+YM;
		}
		...
		function sale_report(TEAM_ID,TYPE,USERNAME){
			window.location.href = ["<%=path%>/company/sale_report?OP=999&TEAM_ID=", TEAM_ID, "&TYPE=", TYPE, "&USERNAME=", USERNAME].join("");
		}
...
//			test2.innerHTML = tplt2(JSON.parse(arr));
		...
		function team_detail(TEAM_ID, TYPE,USERNAME){
			...
			ims_submit_ajax("<%=path%>/company/team_manage", params, vp_detail_cb);
		...
		function close_vp_team(TEAM_ID){
			if(confirm("确定要关闭该副总裁团队吗？")){
				...
				ims_submit_ajax("<%=path%>/company/team_manage", params, close_vp_team_cb);		
			}
		}
		...
		function del_vp_team(TEAM_ID){
			if(confirm("确定要删除该副总裁团队吗？")){
				...
				ims_submit_ajax("<%=path%>/company/team_manage", params, del_vp_team_cb);	
			}
		}
		...
		function adjust(TEAM_ID,ROLE, NAME){
			...
			var params = ["method=vp_adjust_list&TEAM_ID=", TEAM_ID,"&ROLE=",ROLE].join("");
			ims_submit_ajax("<%=path%>/company/team_manage", params, adjust_cb);
		}
		...
		function pagination(current_page,page_size){
			window.location.href="<%=path%>/company/team_manage?method=999&current_page="+current_page+"&page_size="+page_size;
		}
		
		var arr0 = [["5","每页5条"],["10","每页10条"],["15","每页15条"],["20","每页20条"]];
		$( "#PAGE_SIZE" ).change(function() {
			var page_size=this.value;
			window.location.href="<%=path%>/company/team_manage?method=999&current_page=1&page_size="+page_size;
		});
		...
		var cols0 = [
			{id:"form", 
				submit: {
					...
					url: '<%=path%>/company/team_manage?method=vp_team_add',
		...
		var cols1 = [
			...
			submit: {
				postform: submitWindow,
				url: '<%=path%>/company/team_manage?method=manager_team_add',
		...
		var cols2 = [
			...
			submit: {
				...
				postform: submitWindow,
				url: '<%=path%>/company/team_manage?method=sales_member_move',
		...
		var cols3 = [
			...
			submit: {
				...
				postform: submitWindow,
				url: '<%=path%>/company/team_manage?method=manager_team_move',
		...
		function add_sales(VP_ID,TEAM_ID,USERNAME){
			...
			ims_submit_ajax("<%=path%>/company/team_manage", params, no_m_team_sales_cb);
		...
		var cols4 = [
			...
			submit: {
				...
				postform: submitWindow,
				url: '<%=path%>/company/team_manage?method=manager_team_sales_add',
		...
		function team_move_in(TEAM_ID,USER_ID){
			if(confirm("确定将该团队迁入吗？")){
				...
				ims_submit_ajax("<%=path%>/company/team_manage", params, team_move_in_cb);
			}
		}
		...
		function team_close(TEAM_ID,USER_ID) {
			if (confirm("确定要执行关闭操作吗？")) {
				...
				ims_submit_ajax("<%=path%>/company/team_manage", params, sales_cb);
			}
		}
