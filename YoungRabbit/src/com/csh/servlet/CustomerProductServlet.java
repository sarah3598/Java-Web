...
public class CustomerProductServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 注意此处不要验证用户是否已经登录，此处是针对顾客页面的产品，取出信息时注意只取出相关的
	...
	public void getProductDetail(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Object[] param = new Object[1];
		param[0] = GOOD_ID;
		String sql = "SELECT A.*,B.NOW_STOCK_QUANTITY FROM WEB_GOODS_BASIC_INFO A LEFT JOIN WEB_GOODS_INVENTORY B ON A.GOOD_ID=B.GOOD_ID WHERE A.GOOD_ID=?";
	...
	public void findProductList(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.getRequestDispatcher("/zh_CN/customer/yb_product_page.jsp").forward(request, response);
	}
	...
	public JSONArray getProductListData(HttpServletRequest request) throws NumberFormatException {
		...
		String sql = "SELECT ROWNUM AS RN,A.* FROM(SELECT * FROM WEB_GOODS_BASIC_INFO WHERE DELETE_FLAG=? AND HIDE_MARK=? ORDER BY GOOD_ID DESC)A";
	...
	public void addSingleProductInfo(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String MANAGER_OVERRIDING_S = request.getParameter("MANAGER_OVERRIDING_S");
		String VP_OVERRIDING_S = request.getParameter("VP_OVERRIDING_S");
		...
		String MANAGER_COMMISSION = request.getParameter("MANAGER_COMMISSION");
		String VP_OVERRIDING_M = request.getParameter("VP_OVERRIDING_M");
		String VP_COMMISSION = request.getParameter("VP_COMMISSION");

		// insert product info
		String sql1 = "UPDATE WEB_GOODS_BASIC_INFO SET USER_ID=?,HIDE_MARK=?,TITLE=?,BANNED_COUNTRIES=?,CURRENCY=?,PRODUCT_PRICE=?,COMMISSION_MODEL=?,SALE_COMMISSION=?,MANAGER_OVERRIDING_S=?,VP_OVERRIDING_S=?,OWNERSHIP_COMMISSION=?,WHOLESALE_MODEL=?,WHOLESALE_PRICE=?,AGENT_PRICE=?,LOGIN_ACCOUNT=?,MANAGER_COMMISSION=?,VP_OVERRIDING_M=?,VP_COMMISSION=? WHERE GOOD_ID=?";
		...
		param[8] = MANAGER_OVERRIDING_S;
		param[9] = VP_OVERRIDING_S;
		param[10] = OWNERSHIP_COMMISSION;
		param[11] = WHOLESALE_MODEL;
		param[12] = WHOLESALE_PRICE;
		param[13] = AGENT_PRICE;
		param[14] = LOGIN_ACCOUNT;

		param[15] = MANAGER_COMMISSION;
		param[16] = VP_OVERRIDING_M;
		param[17] = VP_COMMISSION;
		param[18] = GOOD_ID;
	...
	public void productUpdateSingle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// update product info
		String sql1 = "UPDATE WEB_GOODS_BASIC_INFO SET USER_ID=?,HIDE_MARK=?,TITLE=?,BANNED_COUNTRIES=?,CURRENCY=?,PRODUCT_PRICE=?,COMMISSION_MODEL=?,SALE_COMMISSION=?,MANAGER_OVERRIDING=?,VP_OVERRIDING=?,OWNERSHIP_COMMISSION=?,WHOLESALE_MODEL=?,WHOLESALE_PRICE=?,AGENT_PRICE=?,LOGIN_ACCOUNT=?,MANAGER_COMMISSION=?,VP_OVERRIDING_M=?,VP_COMMISSION=? WHERE GOOD_ID=?";
		Object[] param = new Object[19];
		param[0] = user_id;
		param[1] = HIDE_MARK;
		param[2] = TITLE;
		...
		int flag1 = CompanyProductService.addSingleProductInfo(sql1, param);
		String normal_json = "";
		String msg = "";
		// 一个产品的基本信息更新成功
		if (flag1 > 0) {
