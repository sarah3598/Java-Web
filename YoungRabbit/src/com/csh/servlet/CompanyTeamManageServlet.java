 ...
 *
 */
public class CompanyTeamManageServlet extends HttpServlet {
	...
	public void findTeamList(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.getRequestDispatcher("/zh_CN/company/yb_db_sales_team.jsp").forward(request, response);
	...
	public JSONArray getTeamListData(HttpServletRequest request) throws NumberFormatException {
		...
		// 01先取出所有团队
		String sql = "SELECT  a.USER_ID AS TEAM_ID,a.USER_ID,a.USERNAME,a.LAST_NAME,a.ROLE,b.STATUS,b.TYPE,(SELECT COUNT(*) FROM SALES_MEMBER WHERE TEAM_ID=b.TEAM_ID) AS SUM FROM USERS a LEFT JOIN SALES_TEAM b ON a.USER_ID=b.TEAM_ID WHERE b.STATUS=1";
	...
	public JSONArray getVPAdjustListData(HttpServletRequest request) throws NumberFormatException {
		...
		if(ROLE >= 4 && ROLE <= 8){		//组长或者经理
			//查询自己的销售员升级为组长或者经理的人
			//SELECT a.USER_ID,a.USERNAME,a.ROLE FROM USERS a LEFT JOIN SALES_MEMBER b ON a.USER_ID=b.MEMBER_ID WHERE b.TEAM_ID=TEAM_ID AND b.ROLE >=4 AND b. ROLE<=8;
			//TEAM_ID为传入的组长或者经理的ID
			String sql = "SELECT a.USER_ID,a.USERNAME,a.ROLE FROM USERS a LEFT JOIN SALES_MEMBER b ON a.USER_ID=b.MEMBER_ID WHERE b.TEAM_ID=? AND b.ROLE >=4 AND b. ROLE<=8 ORDER BY a.ROLE DESC";			
			Object[] param1 = new Object[1];
			param1[0] = TEAM_ID;			

			
			jsonarr = BaseDao.getRsJSONArray(sql, param1);
			
		}
		else if(ROLE == 16){				//副总裁
			//查询跨级领导的销售员
			//SELECT a.USER_ID,a.USERNAME,a.ROLE FROM USERS a LEFT JOIN SALES_MEMBER b ON a.USER_ID=b.MEMBER_ID WHERE b.TEAM_ID=TEAM_ID AND b.ROLE >=1 AND b. ROLE<=2;
			//TEAM_ID为传入的副总裁的ID
			String sql = "SELECT a.USER_ID,a.USERNAME,a.ROLE FROM USERS a LEFT JOIN SALES_MEMBER b ON a.USER_ID=b.MEMBER_ID WHERE b.TEAM_ID=? AND b.ROLE >=1 AND b. ROLE<=2 ORDER BY a.ROLE DESC";			
			Object[] param1 = new Object[1];
			param1[0] = TEAM_ID;			
